...
process.exit(await negative({
  ...
  patches: [{ file: 'src/route.js', from: 'return twoOpt(start, nearestNeighbour(start, stops), end)',
    to: 'return nearestNeighbour(start, stops)' }],
  args: ['--unit-only', '--unit', 'route.test.mjs'],
  expectRed: ['crossing instance: nearest neighbour crosses itself, 2-opt uncrosses it',
