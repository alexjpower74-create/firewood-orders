...
import {
  customerBalance, deliveryCents, formatMoney, goodsCents, hstCents, owingCents, owingLabel, paidCents, priceOrder,
  stackingCents,
} from '../src/money.js'
...
const birch = { kind: 'wood', name: 'Birch, dry', cut_in: 16, stacking_cents_per_cord: 6000,
...
test('delivery bands: the edge belongs to the lower band; past the last band is outside the area', () => {
  ...
  assert.throws(() => deliveryCents(BANDS, 60.01, null), {
    code: 'outside_area', status: 400, extra: { field: 'pin' },
...
test('zones: the chosen zone fee; no zone is a field error', () => {
  const zones = { mode: 'zones', bands: [], beyond_message: '', zones: [{ id: 'z_town', name: 'Springdale', fee_cents: 0 },
...
test('minimum order: at the line passes, a cent below refuses', () => {
  const p = { kind: 'pellets', name: 'Test pellets', bag_lb: 40, bags_per_ton: 50, bags_per_skid: 70,
  ...
  assert.equal(priceOrder({ ...below, product: { ...p, price_cents: { bag: 11000, ton: null, skid: null } } }, settings)
    .goods_cents, 11000)
  assert.throws(() => priceOrder(below, settings), {
    code: 'below_minimum', extra: { field: 'qty' }, message: 'The smallest order we deliver is $110.00 before delivery.',
  })
  // stacking counts toward the minimum
  const small = { kind: 'wood', name: 'Kindling', cut_in: 16, stacking_cents_per_cord: 6000,
...
test('a whole quote: 1 cord of dry softwood to King\'s Point', () => {
  ...
  assert.deepEqual(q, {
    product_label: 'Mixed softwood, dry', qty_label: '1 cord',
    explain: 'A full cord: a stack 4 feet high, 4 feet wide and 8 feet long (128 cubic feet).',
    wood_cu_in: 221184, pellet_bags: 0, distance_km: 12.9,
    goods_cents: 30000, stacking_cents: 0, delivery_cents: 2500, subtotal_cents: 32500, hst_cents: 4875, total_cents: 37375,
  })
})
...
test('owing to the cent on 300 seeded random ledgers', () => {
  ...
  for (let ledger = 0; ledger < 300; ledger++) {
    ...
    for (let i = 0, n = int(0, 8); i < n; i++) {
      ...
      payments.push({ order_id: onOrder ? orders[int(0, orders.length - 1)].id : null, amount_cents: int(1, 300000),
        voided: rand() < 0.2 ? 1 : 0 })
    }
