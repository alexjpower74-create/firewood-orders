...
test('info: shape per API.md, 4 products, delivery dates start tomorrow and skip Sundays', async () => {
  ...
  assert.deepEqual(
    body.delivery.bands.map((b) => b.label),
    ['Up to 10 km: free', 'Up to 30 km: $25.00', 'Up to 60 km: $50.00'],
  )
  assert.deepEqual(body.load, { cords: 1.5, description: SAMPLE_SETTINGS.load.description })
  assert.deepEqual(
    body.products.map((p) => p.id),
    ['p_softwood_dry', 'p_birch_dry', 'p_softwood_green', 'p_pellets'],
  )
  const soft = body.products[0]
  assert.deepEqual(
    Object.keys(soft).sort(),
    ['cut_in', 'dryness', 'id', 'kind', 'name', 'species', 'split', 'stacking_cents_per_cord', 'units'].sort(),
  )
  assert.deepEqual(
    soft.units.map((u) => [u.unit, u.price_cents, u.wood_cu_in]),
    [
      ['cord', 30000, 221184],
      ['half_cord', 17000, 110592],
      ['face_cord', 12000, 73728],
      ['load', 42000, 331776],
    ],
  )
  assert.equal(
    soft.units[3].explain,
    'A load is what our dump truck carries in one trip, dumped in a pile, not stacked. We count a load as 1.5 cords.',
  )
  assert.deepEqual(
    body.products[1].units.map((u) => u.unit),
    ['cord', 'half_cord', 'face_cord'],
  ) // birch: no load
  assert.deepEqual(
    body.products[2].units.map((u) => u.unit),
    ['cord', 'load'],
  )
  const pel = body.products[3]
  assert.deepEqual(
    pel.units.map((u) => [u.unit, u.price_cents, u.pellet_bags]),
    [
      ['bag', 799, 1],
      ['ton', 36500, 50],
      ['skid', 49900, 70],
...
test('quote: three orders written out by hand', async () => {
  ...
  assert.deepEqual(
    [a.distance_km, a.goods_cents, a.stacking_cents, a.delivery_cents, a.subtotal_cents, a.hst_cents, a.total_cents],
  ...
  assert.deepEqual(
    [b.wood_cu_in, b.goods_cents, b.stacking_cents, b.delivery_cents, b.subtotal_cents, b.hst_cents, b.total_cents],
  ...
  assert.deepEqual(
    [c.pellet_bags, c.goods_cents, c.stacking_cents, c.delivery_cents, c.subtotal_cents, c.hst_cents, c.total_cents],
...
test('quote refusals: below minimum, outside the area, not offered', async () => {
  ...
  assert.deepEqual(
    [r.status, r.body.code, r.body.field, r.body.error],
...
const FIELD_CASES = [
  ...
  [
    'preferred',
    {
      preferred: {
        any: false,
        dates: ['2026-09-15', '2026-09-16', '2026-09-17', '2026-09-18', '2026-09-19', '2026-09-21', '2026-09-22', '2026-09-23'],
      },
    },
  ],
  ['name', { name: '' }],
...
test('a new order: 201, the status page says Requested, owing = total; the customer is found again by phone', async () => {
  ...
  const r = await call('POST', '/api/orders', {
    body: orderBody({ phone: '(709) 555-0142', name: 'Wade R. (SAMPLE)', preferred: { any: false, dates: [WED, '2026-09-17'] } }),
  })
...
test('sign-in: wrong PIN 401 field pin; dealer and driver roles; driver on a dealer route 403; no token 401', async () => {
  ...
  assert.deepEqual(
    [wrong.status, wrong.body.code, wrong.body.field, wrong.body.error],
    [401, 'unauthorized', 'pin', 'That PIN is not right.'],
  )
...
test('board buckets and counts', async () => {
  ...
  const ck = await call('POST', '/api/driver/checkins', {
    token: driver,
  ...
  assert.deepEqual(
    r.body.new.map((o) => o.id),
    [a.id, b.id],
  )
  assert.deepEqual(
    r.body.scheduled.map((o) => [o.id, o.status]),
    [[c.id, 'out_for_delivery']],
  )
  assert.deepEqual(
    r.body.delivered.map((o) => o.id),
    [d.id],
  )
  assert.deepEqual(
    r.body.owing.map((o) => o.id),
    [d.id],
  )
...
test('schedule to a Sunday → 400 field date; past and beyond the window refused', async () => {
  ...
  assert.deepEqual(
    [sun.status, sun.body.code, sun.body.field, sun.body.error],
    [400, 'bad_request', 'date', "We don't deliver on Sundays."],
  )
...
test('capacity: 4 cords on Tue, a fifth refused with exact numbers; a half cord fits to 4.50; then even a face cord is refused', async () => {
  ...
  assert.deepEqual(r.body, {
    ...
    day: {
      date: TUE,
      wood: { used_cu_in: 884736, cap_cu_in: 995328, used_cords: 4, cap_cords: 4.5 },
...
test('moving an order between days frees the first day; unschedule frees it too', async () => {
  ...
  assert.deepEqual(
    route.body.stops.map((s) => s.route_pos),
    [1, 2, 3],
    'Tuesday compacts',
  )
...
test('the race: 8 concurrent schedule calls into an empty day → exactly 4 × 200 and 4 × 409', async () => {
  ...
  const results = await Promise.all(
    orders.map((o) => call('POST', `/api/dealer/orders/${o.id}/schedule`, { token: dealer, body: { date: TUE } })),
  )
...
test('idempotent check-in: same op_id twice → 201 then 200 duplicate; stock and door payment once; another op → 409', async () => {
  ...
  const bad = await call('POST', '/api/driver/checkins', {
    token: driver,
  ...
  const none = await call('POST', '/api/driver/checkins', {
    token: driver,
...
test('original time: at = T with server now T + 45 min keeps T; 2 h in the future is adjusted', async () => {
  ...
  const r = await call('POST', '/api/driver/checkins', {
    token: driver,
    now: later,
  ...
  const r2 = await call('POST', '/api/driver/checkins', {
    token: driver,
    now: later,
  ...
  const r3 = await call('POST', '/api/driver/checkins', {
    token: driver,
    now: later,
...
test('door payment: Cash with no amount pays the owing; e-Transfer of part; Owes records nothing', async () => {
  ...
  const pre = await call('POST', '/api/dealer/payments', {
    token: dealer,
    body: {
      customer_id: (await call('GET', `/api/dealer/orders/${cash.id}`, { token: dealer })).body.customer.id,
      order_id: cash.id,
      amount_cents: 10000,
      method: 'etransfer',
    },
  ...
  const ck = (o, payment) =>
    call('POST', '/api/driver/checkins', { token: driver, body: { op_id: uuid(), order_id: o.id, at: T0, payment } })
  ...
  assert.deepEqual(
    pays.map((p) => [p.amount_cents, p.method, p.source, p.date]),
    [
      [10000, 'etransfer', 'dealer', '2026-09-14'],
      [27375, 'cash', 'door', '2026-09-14'],
    ],
  )
...
test('start: the board shows out for delivery and the status page says Out for delivery; idempotent', async () => {
  ...
  assert.deepEqual(
    board.body.scheduled.map((o) => o.status),
    ['out_for_delivery', 'out_for_delivery', 'scheduled'],
  )
  ...
  assert.deepEqual(
    detail.body.messages.map((m) => m.kind),
    ['on_the_way'],
  )
  assert.equal(
  ...
  assert.deepEqual(d.body.messages, [
    {
      ...
      label: "We're coming",
      text:
        `Hi Wade, this is SAMPLE Wood & Pellets. Your order (1 cord of Mixed softwood, dry) is booked for delivery on ` +
...
test('ledger: running balance on 3 orders and 4 payments (one on account)', async () => {
  ...
  const o3 = await call('POST', '/api/orders', {
    ...
    body: orderBody({ phone, product_id: 'p_pellets', unit: 'bag', qty: 14, place: 'Little Bay' }),
  }) // 157.39
  ...
  assert.deepEqual(
    l.body.entries.map((e) => [e.date, e.kind, e.charge_cents, e.payment_cents, e.balance_cents]),
    [
  ...
  assert.deepEqual(
    [list.body.customers[0].balance_cents, list.body.customers[0].paid_cents, list.body.customers[0].orders],
    [37375, 50239, 3],
  )
})
...
test('route optimize on 6 SAMPLE stops matches the pure module order and total_km', async () => {
  ...
  const expected = optimizeRoute(
    YARD,
    Object.entries(byId).map(([id, p]) => ({ id, lat: p.lat, lng: p.lng })),
    YARD,
  )
  assert.deepEqual(
    r.body.stops.map((s) => s.id),
    expected.map((s) => s.id),
  )
  assert.deepEqual(
    r.body.stops.map((s) => s.route_pos),
    [1, 2, 3, 4, 5, 6],
  )
...
test('route optimize after a delivery keeps the delivered stop first and starts from it', async () => {
  ...
  await call('POST', '/api/driver/checkins', {
    token: driver,
    body: { op_id: uuid(), order_id: delivered, at: T0, payment: { method: 'owes' } },
  })
  const r = await call('POST', `/api/dealer/days/${TUE}/route/optimize`, { token: dealer })
  ...
  assert.deepEqual(
    r.body.stops.slice(1).map((s) => s.id),
    optimizeRoute(from, rest, YARD).map((s) => s.id),
  )
})
...
test('route PUT saves an order and refuses missing, duplicate and foreign ids', async () => {
  ...
  assert.deepEqual(
    (await call('GET', `/api/dealer/days/${TUE}/route`, { token: dealer })).body.stops.map((s) => s.id),
    [ids[2], ids[0], ids[1]],
...
test('photo: PUT png → GET returns the same bytes and type; 415 text/plain; 413 over 5 000 000 bytes; 404 unknown op', async () => {
  ...
  const put = (raw, type, id = op) =>
    call('PUT', `/api/driver/checkins/${id}/photo`, { token: driver, raw, headers: { 'Content-Type': type } })
