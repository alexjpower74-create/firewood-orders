...
const used = (col) => `(SELECT COALESCE(SUM(${col}), 0) FROM orders WHERE delivery_date = ?1 AND id <> ?2 AND status IN \${DAY_STATUSES})\n`
...
process.exit(
  await negative({
    ...
    expectRed: [
      'capacity: 4 cords on Tue, a fifth refused with exact numbers; a half cord fits to 4.50; then even a face cord is refused',
