...
export function unitVolume(product, unit, loadCuIn) {
  if (product.kind === 'wood') {
    const v = { cord: CU_IN_PER_CORD, half_cord: CU_IN_PER_CORD / 2, face_cord: FACE_CORD_CU_IN_PER_INCH * product.cut_in, load: loadCuIn }[
      unit
    ]
...
export function explain(product, unit, load) {
  switch (unit) {
    case 'cord':
      return 'A full cord: a stack 4 feet high, 4 feet wide and 8 feet long (128 cubic feet).'
    case 'half_cord':
      return 'Half a cord: 64 cubic feet, half of a full cord.'
    case 'face_cord':
      return (
        `A face cord: one row 4 feet high and 8 feet long, as deep as the pieces are long (${product.cut_in} inches). ` +
        `That is ${faceFraction(product.cut_in)} of a full cord.`
      )
    case 'load':
      return `${load.description} We count a load as ${trimDecimal(load.cords)} cords.`
    case 'bag':
      return `One ${product.bag_lb} lb bag.`
    case 'ton':
      return `A ton: ${product.bags_per_ton} bags of ${product.bag_lb} lb.`
    case 'skid':
      return `A skid: ${product.bags_per_skid} bags of ${product.bag_lb} lb, shrink-wrapped on a pallet.`
  }
