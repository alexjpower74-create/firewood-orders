...
export const SAMPLE_SETTINGS = {
  ...
  delivery: {
    mode: 'bands',
    bands: [{ up_to_km: 10, fee_cents: 0 }, { up_to_km: 30, fee_cents: 2500 }, { up_to_km: 60, fee_cents: 5000 }],
    zones: [],
...
export const SAMPLE_PINS = {
  dealer: { pin: '1357', salt: '5a4d504c45446561c1e7d0f1a2b3c4d5',
    hash: '2cfa6aa78c2e2e43c6c45cc496b109305c3d04b1a113f6c5bff32f90fa16cdd7' },
  driver: { pin: '2580', salt: '5a4d504c45447276e8f9a0b1c2d3e4f5',
...
const wood = (id, name, species, split, prices, stacking, stockCords, sort) => ({
  id, kind: 'wood', name, species, dryness: name.endsWith('green') ? 'green' : 'dry', cut_in: 16, split: split ? 1 : 0,
  stacking_cents_per_cord: stacking, price_cord: prices[0], price_half_cord: prices[1], price_face_cord: prices[2],
  price_load: prices[3], stock_cu_in: cordsToCuIn(stockCords), stock_bags: 0, active: 1, sort,
})
...
export const SAMPLE_PRODUCTS = [
  ...
  { id: 'p_pellets', kind: 'pellets', name: 'SAMPLE Premium wood pellets', brand: 'SAMPLE Premium', bag_lb: 40,
    bags_per_ton: 50, bags_per_skid: 70, price_bag: 799, price_ton: 36500, price_skid: 49900, stock_cu_in: 0,
    stock_bags: 600, active: 1, sort: 4 },
]

export const PRODUCT_COLUMNS = ['id', 'kind', 'name', 'species', 'dryness', 'cut_in', 'split', 'stacking_cents_per_cord',
  'price_cord', 'price_half_cord', 'price_face_cord', 'price_load', 'brand', 'bag_lb', 'bags_per_ton', 'bags_per_skid',
  'price_bag', 'price_ton', 'price_skid', 'stock_cu_in', 'stock_bags', 'active', 'sort']
...
export function sampleStatements() {
  ...
  const out = [{
    sql: 'INSERT INTO settings (id, data, load_cu_in, cap_cu_in, cap_bags, dealer_pin_hash, dealer_pin_salt, ' +
      'driver_pin_hash, driver_pin_salt, updated_at) VALUES (1, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    params: [JSON.stringify(s), cordsToCuIn(s.load.cords), cordsToCuIn(s.truck.wood_cords_per_day),
      s.truck.pellet_skids_per_day * bagsPerSkid, SAMPLE_PINS.dealer.hash, SAMPLE_PINS.dealer.salt,
      SAMPLE_PINS.driver.hash, SAMPLE_PINS.driver.salt, SAMPLE_TIME],
  }]
