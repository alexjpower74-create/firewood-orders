...
export const SEED_PEOPLE = [
  ['Wade R. (SAMPLE)', "King's Point"], ['Mary O. (SAMPLE)', 'South Brook'], ['Gord P. (SAMPLE)', "Robert's Arm"],
  ['Lorraine B. (SAMPLE)', "Pilley's Island"], ['Kevin H. (SAMPLE)', 'Little Bay'], ['Donna W. (SAMPLE)', 'Beachside'],
  ['Barry S. (SAMPLE)', "St. Patrick's"], ['Joan T. (SAMPLE)', "Harry's Harbour"], ['Clarence F. (SAMPLE)', 'Middle Arm'],
  ['Sheila M. (SAMPLE)', "Jackson's Cove"], ['Darrell K. (SAMPLE)', 'Sheppardville'], ['Patsy N. (SAMPLE)', 'Triton'],
]
...
export async function seedDemo(c) {
  ...
  const onOrBefore = (d) => { while (!wd.includes(isoWeekday(d))) d = addDays(d, -1); return d }
  const onOrAfter = (d) => { while (!wd.includes(isoWeekday(d))) d = addDays(d, 1); return d }
  const today = c.today
  ...
  const customers = SEED_PEOPLE.map(([name, placeName], i) => {
    ...
    return { id: `c_seed${String(i + 1).padStart(2, '0')}`, name, phone: `709-555-01${20 + i}`,
      lat: Math.round((pl.lat + LAT_OFF[i % 3]) * 1e6) / 1e6, lng: Math.round((pl.lng + LNG_OFF[i % 3]) * 1e6) / 1e6,
      address: `${SPOTS[i % 3]}, ${placeName}`, dump_notes: DUMPS[i % 3] }
  })
  ...
  const plan = [
    ...
    { who: 5, p: G, unit: 'cord', qty: 1, status: 'delivered', date: ago(3), door: { method: 'owes' }, photo: true,
    ...
    { who: 7, p: W, unit: 'cord', qty: 1, status: todayDelivers ? 'out_for_delivery' : 'scheduled',
      date: todayDelivers ? today : tomorrow },
    { who: 8, p: W, unit: 'half_cord', qty: 1, status: 'cancelled', created: addDays(today, -6),
      payments: [{ amount: 5000, method: 'etransfer', date: addDays(today, -6), note: 'Deposit' }] },
    { who: 9, p: W, unit: 'cord', qty: 1, status: 'scheduled', date: busy,
    ...
    { who: 3, p: W, unit: 'cord', qty: 2, status: 'requested', created: addDays(today, -1),
  ...
  const orders = plan.map((e, k) => {
    ...
    return {
      plan: e, k, id: `o_seed${String(k + 1).padStart(2, '0')}`, token: randomToken(), status: e.status,
      source: k % 3 === 2 ? 'phone' : 'online', customer: cu, product, q, lat: cu.lat, lng: cu.lng,
      delivery_date: onDay ? e.date : null, route_pos: null,
  ...
  for (const date of new Set(orders.filter((o) => o.delivery_date).map((o) => o.delivery_date))) {
    ...
    const rest = optimizeRoute(start, day.filter((o) => o.status !== 'delivered'), s.yard)
    ;[...done, ...rest].forEach((o, i) => { o.route_pos = i + 1 })
  }

  const db = c.db
  const stmts = customers.map((cu) => db.prepare('INSERT INTO customers (id, name, phone, phone_key, created_at) VALUES (?, ?, ?, ?, ?)')
  ...
  for (const o of orders) {
    ...
    stmts.push(db.prepare(`INSERT INTO orders (id, token, status, source, customer_id, product_id, kind, product_label, unit, qty,
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, ?, ?, '', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .bind(o.id, o.token, o.status, o.source, o.customer.id, o.product.id, o.product.kind, q.product_label, e.unit, e.qty,
        q.explain, e.stacking ? 1 : 0, q.wood_cu_in, q.pellet_bags, o.lat, o.lng, q.distance_km, o.customer.address,
        o.customer.dump_notes, preferredAny ? 1 : 0, JSON.stringify(e.preferred || []), q.goods_cents, q.stacking_cents,
        q.delivery_cents, q.subtotal_cents, q.hst_cents, q.total_cents, o.delivery_date, o.route_pos, o.delivered_at,
        e.door ? e.door.method : null, opId, photoKey, photoKey ? 'image/png' : null, o.created_at,
        o.delivered_at || o.cancelled_at || o.created_at, o.cancelled_at))
    let paid = 0
    if (o.status === 'delivered') {
      ...
      stmts.push(
        ...
        db.prepare(`INSERT INTO stock_moves (product_id, change, reason, order_id, checkin_op_id, at) VALUES (?, ?, 'delivered', ?, ?, ?)`)
