...
export function productFromRow(r) {
  ...
  if (r.kind === 'wood') {
    return {
      ...
      stock_cu_in: r.stock_cu_in,
      stock_cords: cordsOf(r.stock_cu_in),
    }
  }
  return {
    ...base,
    brand: r.brand,
    bag_lb: r.bag_lb,
    bags_per_ton: r.bags_per_ton,
    bags_per_skid: r.bags_per_skid,
    price_cents: { bag: r.price_bag, ton: r.price_ton, skid: r.price_skid },
    stock_bags: r.stock_bags,
  }
}
...
export function publicProduct(p, settings) {
  const units = unitsFor(p.kind)
    .filter((u) => p.price_cents[u] !== null)
    .map((u) => {
...
export function deliveryView(delivery) {
  return {
    mode: delivery.mode,
    bands: delivery.bands.map((b) => ({ ...b, label: bandLabel(b) })),
...
export const ORDER_SELECT = 'SELECT o.*, c.name AS name, c.phone AS phone FROM orders o JOIN customers c ON c.id = o.customer_id'
...
export function statusLabel(o) {
  switch (o.status) {
    case 'requested':
      return 'Requested'
    case 'scheduled':
      return `Scheduled for ${longLabel(o.delivery_date)}`
...
export function orderSummary(o, payments) {
  return {
    ...
    cords: cordsOf(o.wood_cu_in),
    preferred_label: preferredLabel(o),
    preferred_dates: preferredDates(o),
    preferred_any: !!o.preferred_any,
    delivery_date: o.delivery_date,
    delivery_label: o.delivery_date ? shortLabel(o.delivery_date) : null,
...
export function customerOrderView(o, payments, settings) {
  ...
  return {
    ...
    order: {
      ...
      paid_cents: paidCents(o.id, payments),
      owing_cents: owing,
      owing_label: owingLabel(owing, { cancelled: o.status === 'cancelled' }),
      created_label: dateTimeLabel(o.created_at),
    },
  }
}
...
export function orderMessages(o, payments, settings, origin) {
  ...
  if (o.status === 'scheduled') {
    out.push({
      ...
      label: "We're coming",
      text:
        `${who} Your order (${what}) is booked for delivery on ${longLabel(o.delivery_date)}. ` +
