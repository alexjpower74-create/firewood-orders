...
const PLACES = {
  "King's Point": [49.5807861, -56.208],
  'South Brook': [49.4029722, -56.089675],
  "Robert's Arm": [49.4766556, -55.8347694],
  'Little Bay': [49.5992389, -55.9518361],
  "St. Patrick's": [49.566667, -55.9825],
  'Middle Arm': [49.7030277, -56.1148445],
  "Jackson's Cove": [49.6852778, -55.9936111],
  Sheppardville: [49.4508334, -56.4316666],
  'Baie Verte': [49.9087083, -56.1946584],
}
...
const SETTINGS = {
  ...
  deposit_text:
    'This page takes no payments. To pay a deposit, send an Interac e-Transfer to sample-wood@example.com with your name in the message. You can also pay cash or e-Transfer when we deliver.',
...
const PRODUCTS = [
  {
    ...
    species: 'Spruce and fir',
    dryness: 'dry',
    cut_in: 16,
    split: true,
    stacking_cents_per_cord: 6000,
    price: { cord: 30000, half_cord: 17000, face_cord: 12000, load: 42000 },
  },
  {
    id: 'p_birch_dry',
    kind: 'wood',
    ...
    split: true,
    stacking_cents_per_cord: 6000,
    price: { cord: 37500, half_cord: 20000, face_cord: 14000, load: null },
  },
  {
    id: 'p_softwood_green',
    kind: 'wood',
    name: 'Mixed softwood, green',
    species: 'Spruce and fir',
    dryness: 'green',
    cut_in: 16,
    split: false,
    stacking_cents_per_cord: null,
    price: { cord: 22000, half_cord: null, face_cord: null, load: 30000 },
  },
  {
    id: 'p_pellets',
    kind: 'pellets',
    ...
    bags_per_ton: 50,
    bags_per_skid: 70,
    price: { bag: 799, ton: 36500, skid: 49900 },
  },
]

const UNIT_NAME = {
  cord: ['cord', 'cords'],
  half_cord: ['half cord', 'half cords'],
  face_cord: ['face cord', 'face cords'],
  load: ['load', 'loads'],
  bag: ['bag', 'bags'],
  ton: ['ton', 'tons'],
  skid: ['skid', 'skids'],
...
function publicProduct(p) {
  const units = Object.keys(p.price)
    .filter((u) => p.price[u] !== null)
    .map((u) => unitInfo(p, u))
  const { price, ...rest } = p
  return { ...rest, units }
}
...
function price(body) {
  ...
  if (!(body.unit in p.price) || p.price[body.unit] === null)
    return err(400, 'not_offered', "We don't sell that by that amount.", { field: 'unit' })
  ...
  if (goods + stacking < SETTINGS.min_order_cents)
    return err(400, 'below_minimum', `The smallest order we deliver is ${money(SETTINGS.min_order_cents)} before delivery.`, {
      field: 'qty',
    })
  ...
  return {
    ...
    body: {
      ...
      qty_label: qtyLabel(body.unit, body.qty),
      explain: u.explain,
      wood_cu_in: wood,
      pellet_bags: bags,
...
function seed() {
  ...
  const mk = (name, phone, placeName, product_id, unit, qty, status, extra = {}) => {
    ...
    if (!c) {
      c = { id: `c_${++seq}`, name, phone, digits }
      s.customers.push(c)
    }
    const q = price({ product_id, unit, qty, lat, lng }).body
    const o = {
      id: `o_${++seq}`,
      token: extra.token || `mock-${seq}-token-sample-0000`,
      status,
      source: extra.source || 'online',
      customer_id: c.id,
      address: `Near ${placeName} (SAMPLE)`,
      dump_notes: extra.dump_notes ?? 'By the woodshed, not on the lawn',
      lat,
      lng,
      product_id,
      unit,
      qty,
      stacking: false,
      preferred_any: !extra.dates,
      preferred_dates: extra.dates || [],
      note: '',
      delivery_date: extra.date || null,
      route_pos: null,
      delivered_at: extra.delivered_at || null,
      door_payment: extra.door || null,
      has_photo: false,
      created_at: extra.created_at || '2026-09-13T14:10:00.000Z',
      ...q,
    }
    s.orders.push(o)
    return o
  }
  mk('Wade R. (SAMPLE)', '709-555-0142', "King's Point", 'p_softwood_dry', 'cord', 1, 'requested', {
    token: 'demo-requested-sample',
  ...
  mk('Gerald P. (SAMPLE)', '709-555-0163', "St. Patrick's", 'p_birch_dry', 'half_cord', 1, 'requested', {
    source: 'phone',
    created_at: '2026-09-14T11:02:00.000Z',
  })
  const a = mk('Darlene K. (SAMPLE)', '709-555-0128', "Robert's Arm", 'p_softwood_dry', 'cord', 2, 'scheduled', {
    token: 'demo-scheduled-sample',
    date: '2026-09-15',
  })
  ...
  b.route_pos = 2
  c.route_pos = 1
  mk('Leonard S. (SAMPLE)', '709-555-0171', "Jackson's Cove", 'p_softwood_dry', 'face_cord', 2, 'out_for_delivery', {
    token: 'demo-out-sample',
    date: TODAY,
  }).route_pos = 1
  const d1 = mk('Patsy W. (SAMPLE)', '709-555-0134', 'Sheppardville', 'p_birch_dry', 'cord', 1, 'delivered', {
    token: 'demo-delivered-sample',
    date: '2026-09-11',
    delivered_at: '2026-09-11T17:05:00.000Z',
    door: 'owes',
  })
  const d2 = mk('Rodney T. (SAMPLE)', '709-555-0182', 'Little Bay', 'p_softwood_dry', 'cord', 1, 'delivered', {
    date: '2026-09-12',
    delivered_at: '2026-09-12T15:40:00.000Z',
    door: 'cash',
  })
  d1.route_pos = 1
  d2.route_pos = 1
  ...
  s.payments.push({
    id: `pay_${++seq}`,
    customer_id: d2.customer_id,
    order_id: d2.id,
    amount_cents: d2.total_cents,
    method: 'cash',
    date: '2026-09-12',
    note: '',
    source: 'door',
...
function createdLabel(o) {
  const l = instantLabel(o.created_at)
  return `${l.day}, ${l.time}`
}
...
function summary(s, o) {
  ...
  return {
    ...
    kind: PRODUCTS.find((p) => p.id === o.product_id).kind,
    product_label: o.product_label,
    unit: o.unit,
    qty: o.qty,
    qty_label: o.qty_label,
    stacking: o.stacking,
    wood_cu_in: o.wood_cu_in,
    pellet_bags: o.pellet_bags,
    cords: Number(cords2(o.wood_cu_in)),
    preferred_label: preferredLabel(o),
    preferred_dates: o.preferred_dates,
    preferred_any: o.preferred_any,
    delivery_date: o.delivery_date,
    delivery_label: o.delivery_date ? label(o.delivery_date) : null,
    route_pos: o.route_pos,
    distance_km: o.distance_km,
    goods_cents: o.goods_cents,
    stacking_cents: o.stacking_cents,
    delivery_cents: o.delivery_cents,
    subtotal_cents: o.subtotal_cents,
    hst_cents: o.hst_cents,
    total_cents: o.total_cents,
    paid_cents: paid,
    owing_cents: owingOf(s, o),
...
function info() {
  return {
    ...
    season_open: !closed(),
    season_message: SETTINGS.season_message,
    deposit_text: SETTINGS.deposit_text,
    min_order_cents: SETTINGS.min_order_cents,
    hst_registered: SETTINGS.hst_registered,
    yard: YARD,
...
function validateOrder(_s, body, { dealer = false } = {}) {
  if (!dealer && closed()) return err(403, 'season_closed', SETTINGS.season_message)
  const q = price(body)
  if (q.status !== 200) return q
  const str = (v) => (typeof v === 'string' ? v.trim() : '')
  if (str(body.address).length < 1 || str(body.address).length > 120)
    return bad('address', 'Tell us where the place is, like the road and what it is near.')
  ...
  if (
    !pref.any &&
    (!Array.isArray(pref.dates) || pref.dates.length < 1 || pref.dates.length > 7 || !pref.dates.every((d) => dates.includes(d)))
  ) {
  ...
  if (!/^[\d\s+\-().]{7,32}$/.test(phone) || phone.replace(/\D/g, '').length < 7)
    return bad('phone', 'Enter a phone number we can call, like 709-555-0142.')
...
function createOrder(s, body, source) {
  ...
  const o = {
    ...
    dump_notes: (body.dump_notes || '').trim(),
    lat: body.lat,
    lng: body.lng,
    ...
    preferred_any: !!body.preferred.any,
    preferred_dates: body.preferred.any ? [] : body.preferred.dates,
    note: (body.note || '').trim(),
    delivery_date: null,
    route_pos: null,
    delivered_at: null,
...
function dayOf(s, date, excludeId = null) {
  const on = s.orders.filter(
    (o) => o.delivery_date === date && o.id !== excludeId && ['scheduled', 'out_for_delivery', 'delivered'].includes(o.status),
  )
  ...
  return {
    ...
    delivers,
    reason: delivers ? null : 'No deliveries on Sundays',
    orders: on.length,
    started: on.some((o) => o.status !== 'scheduled'),
    over: wood > SETTINGS.cap_cu_in || bags > SETTINGS.cap_bags,
    wood: {
      used_cu_in: wood,
      cap_cu_in: SETTINGS.cap_cu_in,
      used_cords: Number(cords2(wood)),
      cap_cords: Number(cords2(SETTINGS.cap_cu_in)),
    },
...
function messages(s, o) {
  ...
  if (o.status === 'scheduled')
    out.push({
      kind: 'scheduled',
      label: "We're coming",
      text: `Hi ${first}, this is ${SETTINGS.short_name}. Your order (${o.qty_label} of ${o.product_label}) is booked for delivery on ${longLabel(o.delivery_date)}. Amount owing: ${money(Math.max(owing, 0))}. Check your order: ${link}`,
    })
  if (o.status === 'out_for_delivery')
    out.push({
      kind: 'on_the_way',
      label: 'On the way',
      text: `Hi ${first}, this is ${SETTINGS.short_name}. We're on the way with your ${o.qty_label} of ${o.product_label}. See you soon.`,
    })
  if (o.status === 'delivered' && owing > 0)
    out.push({
      kind: 'balance',
      label: 'Balance reminder',
      text: `Hi ${first}, this is ${SETTINGS.short_name}. Thanks again for your order. The balance of ${money(owing)} is still owing. ${SETTINGS.deposit_text}`,
    })
  return out
}
...
function compact(s, date) {
  if (!date) return
  s.orders
    .filter((o) => o.delivery_date === date && o.status !== 'cancelled')
    .sort((a, b) => a.route_pos - b.route_pos)
    .forEach((o, i) => {
      o.route_pos = i + 1
    })
}
...
export async function handle(method, path, body, headers) {
    ...
    return {
      ...
      body: {
        ...
        order: {
          ...
          qty_label: v.qty_label,
          explain: o.explain,
          stacking: v.stacking,
          address: v.address,
          dump_notes: v.dump_notes,
          preferred_label: v.preferred_label,
          delivery_date: v.delivery_date,
          delivered_label: v.delivered_label,
          photo_url: o.has_photo ? `/api/photos/${o.token}` : null,
          goods_cents: v.goods_cents,
          stacking_cents: v.stacking_cents,
          delivery_cents: v.delivery_cents,
          subtotal_cents: v.subtotal_cents,
          hst_cents: v.hst_cents,
          total_cents: v.total_cents,
          paid_cents: v.paid_cents,
          owing_cents: v.owing_cents,
          owing_label: owingLabel(v.owing_cents),
  ...
  if (method === 'POST' && p === '/api/signin') {
    ...
    if (pin === '1357')
      return { status: 200, body: { token: 'mock-dealer-token-sample', role: 'dealer', expires_at: '2026-09-14T23:30:00.000Z' } }
    if (pin === '2580')
      return { status: 200, body: { token: 'mock-driver-token-sample', role: 'driver', expires_at: '2026-09-28T11:30:00.000Z' } }
  ...
  if (p.startsWith('/api/dealer/')) {
    ...
    if (method === 'GET' && p === '/api/dealer/board') {
      ...
      const out = {
        ...
        scheduled: sums
          .filter((o) => ['scheduled', 'out_for_delivery'].includes(o.status))
          .sort((a, b) => a.delivery_date.localeCompare(b.delivery_date) || a.route_pos - b.route_pos),
        delivered: sums.filter((o) => o.status === 'delivered').sort((a, b) => b.delivered_at.localeCompare(a.delivered_at)),
        owing: sums
          .filter((o) => o.status === 'delivered' && o.owing_cents > 0)
      ...
      return {
        ...
        body: {
          counts: {
            new: out.new.length,
            scheduled: out.scheduled.length,
            delivered: out.delivered.length,
            owing: out.owing.length,
            cancelled: sums.filter((o) => o.status === 'cancelled').length,
          },
          ...out,
        },
      }
    ...
    if ((m = p.match(/^\/api\/dealer\/orders\/([^/]+)(?:\/(schedule|unschedule))?$/))) {
      ...
      if (method === 'GET' && !m[2]) {
        ...
        const balance =
          mine.filter((x) => x.status !== 'cancelled').reduce((t, x) => t + x.total_cents, 0) -
          s.payments.filter((x) => x.customer_id === c.id && !x.voided).reduce((t, x) => t + x.amount_cents, 0)
        return {
          status: 200,
          body: {
            order: summary(s, o),
            customer: { id: c.id, name: c.name, phone: c.phone, balance_cents: balance },
            payments: s.payments.filter((x) => x.order_id === o.id),
            messages: messages(s, o),
          },
        }
      }
      if (method === 'POST' && m[2] === 'schedule') {
        ...
        if (day.wood.used_cu_in + o.wood_cu_in > SETTINGS.cap_cu_in || day.pellets.used_bags + o.pellet_bags > SETTINGS.cap_bags) {
          ...
          return err(409, 'over_capacity', error, {
            day: { date, wood: day.wood, pellets: day.pellets },
    ...
    if (method === 'POST' && p === '/api/dealer/payments') {
      ...
      if (b.order_id && !s.orders.some((x) => x.id === b.order_id && x.customer_id === c.id))
        return err(404, 'not_found', "We couldn't find that order.")
      if (!Number.isInteger(b.amount_cents) || b.amount_cents < 1 || b.amount_cents > 10000000)
        return bad('amount_cents', 'Enter an amount from $0.01 to $100,000.00.')
