...
export function createOrderForm(root, { info, mode = 'customer', onChange = () => {}, deliveryOverride = () => null }) {
  ...
  <section class="step" data-step="3" aria-labelledby="h-step3">
    ...
    ${
      zones
        ? `<label class="field"><span class="field-label">Delivery zone</span>
    ...
    ${
      dealer
        ? `<label class="field"><span class="field-label">Delivery fee (optional)</span>
  ...
  <section class="step" data-step="4" aria-labelledby="h-step4">
    ...
    <div class="days">${info.delivery_dates
      .map((d) => {
  ...
  function renderUnits() {
    ...
    $('.units').innerHTML = p
      ? p.units
          .map(
            (u) => `<button type="button" class="unit" role="radio" data-unit="${esc(u.unit)}" aria-checked="${u.unit === state.unit}">
          <span class="unit-name">${esc(UNIT_TITLE[u.unit] || u.unit)}</span><span class="unit-price">${money(u.price_cents)} <span>${esc(UNIT_EACH[u.unit] || '')}</span></span></button>`,
          )
          .join('')
      : ''
    const u = unitOf()
  ...
  root.addEventListener('click', (ev) => {
    ...
    if (b.matches('.product')) {
      ...
      if (changed) {
        state.unit = product().units[0]?.unit || null
        state.qty = 1
        $('#qty').textContent = '1'
      }
  ...
  $('#zone')?.addEventListener('change', (ev) => {
    state.zone_id = ev.target.value || null
    requote()
  })
  for (const input of root.querySelectorAll('.input')) {
    input.addEventListener('input', () => {
      const slot = input.closest('.field')?.querySelector('[data-error-for]')
      if (slot && !QUOTE_FIELDS.includes(slot.dataset.errorFor)) {
        slot.hidden = true
        slot.textContent = ''
  ...
  function mountMap() {
    ...
    L.circleMarker([info.yard.lat, info.yard.lng], { radius: 8, color: '#1a0e02', weight: 2, fillColor: '#fb923c', fillOpacity: 1 })
      .addTo(map)
      .bindTooltip(esc(info.yard.label))
  ...
  function body() {
    ...
    return {
      ...
      lat: state.pin?.lat ?? null,
      lng: state.pin?.lng ?? null,
      address: val('#address'),
      dump_notes: val('#dump-notes'),
  ...
  async function validateStep(n) {
    ...
    if (n === 1) {
      if (!state.product) {
        showError(section, 'product_id', 'Choose what you would like.')
        return false
      }
      return true
    }
    ...
    if (n === 3) {
      if (!state.pin) {
        showError(section, 'pin', 'Tap the map where the truck should dump it.')
        return false
      }
      await settle()
      if (state.quoteError) {
      ...
      if (zones && !state.zone_id) {
        showError(section, 'zone_id', 'Pick your area.')
        return false
      }
      if (!body().address) {
        showError(section, 'address', 'Tell us where to find the place, like the road and what it is near.')
        return false
      }
      return true
    }
    if (n === 4) {
      if (!state.any && !state.dates.length) {
        showError(section, 'preferred', 'Pick the days that suit you, or Any day.')
        return false
      }
      return true
    }
  ...
  function renderPrice(container) {
    ...
    if (!p) {
      ...
      const item = (label, value, id) =>
        `<span class="price-item"${id ? ` id="${id}"` : ''}><span>${label}</span> <strong>${value}</strong></span>`
      ...
      lines = [
        ...
        pinned
          ? item('Delivery', q.delivery_cents === 0 ? 'Free' : money(q.delivery_cents), 'quote-delivery')
          : item('Delivery', 'after the map pin', 'quote-delivery'),
