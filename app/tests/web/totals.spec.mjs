...
test('September matches the API and a hand-worked HST, and the CSV downloads equal the API files byte for byte', async ({ page, context, request }, testInfo) => {
  ...
  const b = await orderViaApi(request, { product_id: 'p_pellets', unit: 'bag', qty: 14, name: 'Maureen B. (SAMPLE)', phone: '709-555-0117' })
  ...
  const paid = await api(request, 'POST', '/api/dealer/payments', { customer_id: bCustomer, order_id: b.id, amount_cents: 2000, method: 'etransfer' }, bearer(dealer))
  expect(paid.status).toBe(201)

  const hand = {
    delivered: 2, goods_cents: 30000 + 14 * 799, stacking_cents: 0, delivery_cents: 5000,
    subtotal_cents: 32500 + 13686, hst_cents: hstOf(32500) + hstOf(13686), payments_cents: aTotal + 2000,
  }
